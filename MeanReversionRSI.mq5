...
/*******************
@Brief - Mean Reversion RSI Strategy
RSI Entry Criteria for SELL 
 ⁃ RSI 14 (Short term momentum confirmation) 
 ⁃ Higher than 70% 
 ⁃ RSI 3 (Ultra short momentum/signal trigger) 
 ⁃ Higher than 80% 

RSI Better Entry Criteria for SELL 
 ⁃ RSI 14 (Short term momentum confirmation) 
 ⁃ Higher than 85% 
 ⁃ RSI 3 (Ultra short momentum/signal trigger) 
 ⁃ Higher than 95% 
 ⁃ RSI 200/250 is lower than 47 

————————————

RSI Entry Criteria for BUY 
 ⁃ RSI 14 (Short term momentum confirmation) 
 ⁃ Lower than 30% 
 ⁃ RSI 3 (Ultra short momentum/signal trigger) 
 ⁃ Lower than 20% 

 ⁃ RSI 200/250 (Long term trend filter) 
 ⁃ Used to observe the long term trend, if it’s above 50% (Uptrend) if below 50% (Downtrend) 

RSI Better Entry Criteria for BUY 
 ⁃ RSI 14 (Short term momentum confirmation) 
 ⁃ Lower  than 30% 
 ⁃ RSI 3 (Ultra short momentum/signal trigger) 
 ⁃ Lower than 20% 
 ⁃ RSI 200/250 is higher than 53 

————————————

Entry Criteria based on momentum 
 ⁃ Make sure there is 3 consecutive candlestick formation on the reverse
 ⁃ Reversal candle should be higher or lower than 3 previous candles
*******************/
input string Mean_Reversion_RSI_Strategy_Settings = "***MEAN REVERSION RSI TRADING STRATEGY SETTINGS***";
input int RSIPeriod3 = 3;
input int RSIPeriod14 = 14;
input int RSIPeriod200 = 200;
input int RSIPeriod250 = 250;
input int Consecutive_Candlestick_Formation = 3;
input int Reversal_Candlestick_Formation = 3;
void Strategy_MeanReversionRSI(){
   tradeMethod = "Mean Reversion RSI";//using Mean Reversion RSI trading strategy
   
   //if there are currently no positions open
   int RSIHandle3 = iRSI(_Symbol,_Period,RSIPeriod3,PRICE_CLOSE); //definition 
   int RSIHandle14 = iRSI(_Symbol,_Period,RSIPeriod14,PRICE_CLOSE); //definition 
   int RSIHandle200 = iRSI(_Symbol,_Period,RSIPeriod200,PRICE_CLOSE); //definition 
   int RSIHandle250 = iRSI(_Symbol,_Period,RSIPeriod250,PRICE_CLOSE); //definition 
   double RSIArray3[];
   double RSIArray14[];
   double RSIArray200[];
   double RSIArray250[];
   CopyBuffer(RSIHandle3,0,1,1,RSIArray3); //Set data
   CopyBuffer(RSIHandle14,0,1,1,RSIArray14); //Set data
   CopyBuffer(RSIHandle200,0,1,1,RSIArray200); //Set data
   CopyBuffer(RSIHandle250,0,1,1,RSIArray250); //Set data
   double RSIValue3 = RSIArray3[0]; //RSI 3 Value
   double RSIValue14 = RSIArray14[0]; //RSI 14 Value
